...
if($catch_user_id != "")
{
    ...
    {   
        ...
        {
            $week_query = mysqli_query($my_db, "SELECT * FROM L_schedule WHERE l_id = '" . $catch_lecture['l_id'] . "'");

            $catch_week = mysqli_fetch_array($week_query);
            $week_sum = $catch_week['week'];

            while($catch_week = mysqli_fetch_array($week_query))
            {
                $week_sum .= "," . $catch_week['week'];
            }

            echo $catch_lecture['l_id'] . "∥" . 
                 $catch_lecture['l_name'] . "∥" .
                 $week_sum . "┃" ;
        }
    }
